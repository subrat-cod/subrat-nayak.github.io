# 🌐 Live Portfolio: **[subrat-cod.github.io/subrat-nayak.github.io](https://subrat-cod.github.io/subrat-nayak.github.io/)** 🔥

# About Me 🚀

Hello! I am **Subrat Nayak**, a First-Year B.Tech student at the **Odisha University of Technology and Research (OUTR), Bhubaneswar**, currently pursuing my degree in **Artificial Intelligence & Robotics**.

I am a passionate **Data Analyst** and an aspiring **Full-Stack Developer** who loves exploring DevOps and Generative AI. I believe in continuous learning and building high-performance data processing pipelines.

---

## 💻 Visual Preview of My Portfolio

Here is a glimpse of my live portfolio website:

### 🌅 Hero Section & Welcome Interface
<img src="https://raw.githubusercontent.com/subrat-cod/subrat-nayak.github.io/main/image%20copy.png" alt="Portfolio Home Section" width="80%">



--

## 🛠️ What I Do
* **Frontend Web Development:** Building modern UI layouts, responsive designs, and interactive 3D elements.
* **Data Analytics:** Transforming raw data into meaningful business insights and creating data pipelines.

## 🧰 Web Technologies & Skills
* **Languages & Web:** `HTML5` | `CSS3` | `JavaScript` | `C++`
* **Data Science & Tools:** `Python` | `SQL` | `Excel` | `Power BI` | `Docker`

---

## 🤝 Connect With Me
* **Website:** [subrat-nayak.github.io](https://subrat-nayak.github.io)
* **GitHub:** [subrat-cod](https://github.com/subrat-cod)
*: screen shot:
<img width="756" height="1600" alt="image" src="https://github.com/user-attachments/assets/9f72f1df-ba88-48d3-a800-eeb7a180e1a5" />
